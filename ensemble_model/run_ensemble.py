...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="native_masking",
        choices=["native_masking", "hybrid_adaptive"],
        help="Which Transformer scenario to use for inference (default: native_masking)",
    )
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    print("\n[Phase 1] Loading data and building splits...")
    df = load_and_prepare(args.dataset_path)
    ...
    obj2_df = build_obj2_comparison(summary_df, output_path=OBJ2_COMPARE_CSV)
    ...
    best_row = obj2_df.iloc[0]
    print(f"\n★  Best model: {best_row['model']}  |  AUC = {best_row['auc_mean']:.4f}")
    print("=" * 72)
